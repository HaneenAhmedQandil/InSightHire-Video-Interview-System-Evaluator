...
#             score_text = f"Overall Interview Score: {aggregate_score}/100"
#             story.append(Paragraph(score_text, score_style))
#             story.append(Spacer(1, 0.4*inch))
            
#             # Component breakdown
#             story.append(Paragraph("<b>Component Performance Breakdown:</b>", self.styles['Heading2']))
...
#             if 'old_score' in evaluation_data and evaluation_data['old_score']:
...
#                 rubric_text = f"<b>Rubric Score:</b> {rubric_score}/100"
#                 story.append(Paragraph(rubric_text, self.styles['ScoreStyle']))
#                 story.append(Spacer(1, 0.2*inch))
                
#                 # Rubric criteria table
#                 scores = rubric_breakdown.get('scores', [])
#                 if scores:
#                     story.append(Paragraph("<b>Detailed Rubric Breakdown:</b>", self.styles['Heading2']))
...
#                         table_data.append([
#                             score_item.get('name', ''),
#                             f"{score_item.get('score', 0)}/100",
#                             score_item.get('explanation', '')[:80] + ('...' if len(score_item.get('explanation', '')) > 80 else '')
#                         ])
...
#                 name='FinalScoreStyle',
#                 parent=self.styles['LargeScore'],
#                 textColor=final_color
#             )
            
#             final_text = f"<b>Final Combined Score: {final_score}/100</b>"
#             story.append(Paragraph(final_text, final_style))
...
#             aggregate_score = self.calculate_aggregate_score(analysis_results)
        
#         # Large colorized score box
#         score_color = self.get_score_color(aggregate_score)
        
#         aggregate_style = ParagraphStyle(
#             name='AggregateStyle',
#             parent=self.styles['LargeScore'],
#             fontSize=32,
#             textColor=score_color,
...
#         aggregate_text = f"Overall Aggregate Score: {aggregate_score:.1f}/100"
#         story.append(Paragraph(aggregate_text, aggregate_style))
#         story.append(Spacer(1, 0.4*inch))
...
class PDFReportGenerator:
    ...
    def create_overall_performance_page(self, interview_summary):
        ...
        if interview_summary.get('overall_aggregate'):
            ...
            score_text = f"Overall Interview Score: {aggregate_score:.1f}/100"
            story.append(Paragraph(score_text, score_style))
            story.append(Spacer(1, 0.4*inch))
    ...
    def create_evaluation_page(self, evaluation_data):
        ...
        if evaluation_data:
            ...
            if rubric_breakdown:
                ...
                if scores:
                    ...
                    for item in scores:
                        ...
                        truncated = exp if len(exp) <= 80 else exp[:80] + '...'
                        table_data.append([name, f"{sc}/100", truncated])
            ...
            story.append(Paragraph(f"Final Combined Score: {final_score:.2f}/100", final_style))
